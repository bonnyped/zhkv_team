...
Примечание о жизненном цикле: в этой реализации запуск, пауза и завершение симуляции
реализованы через классы `SimulationRunner`, `SimulationState` и `InputListner`.
`SimulationRunner` запускает поток симуляции и опирается на флаги в `SimulationState`
(`running`/`paused`) для управления выполнением ходов; `InputListner` обрабатывает
ввод пользователя (команда `p` — пауза/возобновление, `q` или `exit` — выход) и
либо переключает флаги в `SimulationState`, либо сигнализирует `SimulationRunner` о
